...
class Root {
  constructor(x, y, color, hue, saturation, centerX, centerY) {
    ...
    this.particleScaleInner = Math.random() * 90 + 30;
  ...
  draw = () => {
    this.saturation -= 0.2;
    this.hue += 0.02;
    this.speedX += (this.direction * Math.sin(this.speedX)) / 2;
    this.speedY += (this.direction * Math.cos(this.speedY)) / 2;
    this.x += this.speedX;
    this.y += this.speedY;
...
const randomDrawing = () => {
  ...
  randNumberOfColors = Math.ceil(Math.random() * 2) + 1;
  numberToAdd = 360 / randNumberOfColors;
  for (let i = 0; i < Math.random() * 700 + 10; i++) {
