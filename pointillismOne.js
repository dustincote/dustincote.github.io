...
music.onended = function () {console.log('music done'), start = true, firstClick= true, shouldStop= false, console.log('start is:', start) }
    ...
    class particle {
        ...
        update() {
            ...
            if (distance < mouse.radius + this.size) {
                ...
                if (this.x < 0) { this.x = Math.random() * canvas.width }
                if (this.x > canvas.width) { this.x = Math.random() * canvas.width }
                if (this.y < 0) { this.y = Math.random() * canvas.height }
                if (this.y > canvas.height) { this.y = Math.random() * canvas.height }
                this.x += (Math.cos(Math.random() * 100)/1.1), this.y += (Math.cos(Math.random() * 100)/1.1)
            }
